...
function createBoard() {
    startPieces.forEach((startPiece, i) => {
        ...
        if (isEvenRow) {
            ...
            square.classList.add(isEvenSquare ? 'brown' : 'beige');
        }
...
function dragDrop(e) {
    e.stopPropagation();
    const correctGo = draggedElement.querySelector('svg').classList.contains(playerGo)
    const taken = e.target.classList.contains('piece')
    const valid = checkIsValid(e.target)
    const opponentGo = playerGo === 'white' ? 'black' : 'white'

    const takenByOpponent = e.target.querySelector('svg')?.classList.contains(opponentGo)
    if (correctGo) {
        ...
        if (taken && !takenByOpponent) {
            infoDisplay.textContent = "U can't go here"
...
function changePlayer() {
    if (playerGo === "black") {
        reverseIds()
        playerGo = "white";
        player.textContent = "white "; // Изменяем текст на "It's black turn"
    } else {
        revertIds()
        playerGo = "black";
        player.textContent = "black "; // Изменяем текст на "It's white turn"
    }
}
...
function checkWin() {
    ...
    const blackKing = kings.find(king => king.querySelector('svg').classList.contains('black'));
    const whiteKing = kings.find(king => king.querySelector('svg').classList.contains('white'));
